#!/usr/bin/env python3
"""
Fit API-based Rainfall Thresholds per Sub-bacia — Seasonal Split

Same methodology as fit_api_thresholds_per_subbacia.py but fits separate
thresholds for each meteorological season:
...
Motivation: a fixed annual threshold causes high FAR in the dry season
because intense rain rarely coincides with saturated soil. Seasonal
thresholds are expected to reduce false alarms in winter/spring.

Outputs:
    api_threshold_parameters_seasonal.csv  — fitted parameters per
        sub-bacia × season × time-step
    api_threshold_metrics_seasonal.csv     — detailed evaluation metrics
"""
...
COMPOSITE_PATH     = f'{INPUT_DIR}/subbacia_daily_composite_reconstructed.csv'
...
# Minimum events per class to attempt fitting
MIN_EA  = 3
MIN_ESA = 3
MIN_TOTAL = 10
...
# ==============================================================================
# HELPER FUNCTIONS  (identical to the annual script)
# ==============================================================================
...
# ==============================================================================
# CORE FITTING FUNCTION  (seasonal-aware)
# ==============================================================================
...
EMPTY_PARAMS = {
    ...
    'best_antecedent_days': 0, 'best_k': np.nan,
...
def fit_season_timestep(season_events, ts_col, ts_label, subbacia_daily_df):
    ...
    y_pred_ur  = (valid[ts_col] >= upper_raw).astype(int)
    y_pred_lr  = (valid[ts_col] >= lower_raw).astype(int)
    y_pred_ut  = (valid[ts_col] >= upper_tol).astype(int)
    y_pred_lt  = (valid[ts_col] >= lower_tol).astype(int)

    met_ur = compute_metrics(y_true, y_pred_ur)
    met_lr = compute_metrics(y_true, y_pred_lr)
    met_ut = compute_metrics(y_true, y_pred_ut)
    met_lt = compute_metrics(y_true, y_pred_lt)
    ...
    if len(middle_events) >= MIN_TOTAL:
        for k in K_VALUES:
            middle_api = compute_api_for_k(middle_events, subbacia_daily_df, k)
            full_api   = compute_api_for_k(valid,          subbacia_daily_df, k)

            for n_days in range(1, MAX_ANTECEDENT_DAYS + 1):
                api_col  = f'_api_k_{n_days}d'
                ok       = middle_api[api_col].notna()
                me       = middle_api[ok]
                if len(me) < 5:
                    continue
                ...
                if not np.isnan(result['a']):
                    fv   = full_api[full_api[api_col].notna()]
...
def main():
    print("=" * 80)
    print("API THRESHOLD FITTING — PER SUB-BACIA, SEASONAL")
    print("=" * 80)
    ...
    for shi_cd in tqdm(subbacia_ids, desc="Sub-bacias"):
        ...
        sb_daily   = composite_df[composite_df['shi_cd'] == shi_cd]
        ...
        for season in season_order:
            ...
            for ts_idx, ts_col in enumerate(TIME_STEP_COLS):
                ...
                params, detailed = fit_season_timestep(
                    seas_events, ts_col, ts_label, sb_daily
                )
    ...
    params_path = f'{OUTPUT_DIR}/api_threshold_parameters_seasonal.csv'
    ...
    if detailed_all:
        ...
        metrics_path = f'{OUTPUT_DIR}/api_threshold_metrics_seasonal.csv'
    ...
    print(f"  Rows with fitted threshold: {len(fitted)}")
    print()
    print("  Fitted rows per season:")
    print(fitted.groupby('season').size().reindex(season_order).to_string())
    print()
    print("  Mean POD / FAR by season (lower_tol):")
    summary = (fitted.groupby('season')[['POD_lower_tol','FAR_lower_tol']]
