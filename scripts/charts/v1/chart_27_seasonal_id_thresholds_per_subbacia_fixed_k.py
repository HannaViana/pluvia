...
Identical structure to chart_22 but uses seasonal_fixed_k parameters.
"""
...
def create_chart_for_subbacia(shi_cd, shi_nm, sb_events, sb_params):
    ...
    for col_idx, (season, s_label, s_color) in enumerate(
        ...
        n_ea_s  = len(ea)
        n_esa_s = len(esa)
        suffix  = '' if len(upper_vals) > 0 else '\n(no threshold)'
    ...
    fig.suptitle(f'I-D Thresholds by Season (Fixed K=0.85) — {title_name}\n'
...
def main():
    print("=" * 70)
    print("CHART 27: SEASONAL I-D THRESHOLDS PER SUB-BACIA (FIXED K=0.85)")
    print("=" * 70)
    ...
    out_dir = f'{OUTPUT_DIR}/chart_27_seasonal_id_thresholds_fixed_k'
