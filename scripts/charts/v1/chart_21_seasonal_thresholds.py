"""
Chart 21: Seasonal API Threshold Analysis — Summary Charts
Three panels:
  (a) POD / FAR / Score by season (bar chart)
  (b) Distribution of lower_tol by season and time-step (box plot)
  (c) K distribution by season (stacked bar)
"""
...
K_COLORS = ['#053061','#2166ac','#4393c3','#92c5de','#f4a582','#d6604d','#b2182b']
...
def panel_a_metrics(ax, valid, annual_valid):
    """Bar chart: POD, FAR, Score by season + annual reference."""
    ...
    bars_pod   = ax.bar(x - w,     metrics['POD'],   w, label='POD',   color='#2166ac', alpha=0.85)
    bars_far   = ax.bar(x,         metrics['FAR'],   w, label='FAR',   color='#d6604d', alpha=0.85)
    bars_score = ax.bar(x + w,     metrics['Score'], w, label='Score', color='#35978f', alpha=0.85)
...
def panel_b_thresholds(ax, valid, annual_valid):
    """Box plot: lower_tol by season for each time-step."""
    ...
    for s_idx, (season, color) in enumerate(zip(SEASON_ORDER, SEASON_COLORS)):
        ...
        bp = ax.boxplot(data_by_ts, positions=positions, widths=width * 0.7,
    ...
    # Annual mean line per time-step
    for i, ts in enumerate(TS_ORDER):
        x_center = positions_base[i] + (len(SEASON_ORDER) - 1) * width / 2
        ax.plot([x_center - 1.5, x_center + 1.5], [ann_means[ts], ann_means[ts]],
...
def panel_c_k_distribution(ax, valid):
    """Stacked bar: K distribution by season."""
    x = np.arange(len(SEASON_ORDER))
    w = 0.55
    bottoms = np.zeros(len(SEASON_ORDER))

    for k_val, k_color in zip(K_VALUES, K_COLORS):
        counts = []
        for s in SEASON_ORDER:
            sv = valid[valid['season'] == s]['best_k'].dropna()
            pct = (sv == k_val).mean() * 100 if len(sv) > 0 else 0
            counts.append(pct)
        ax.bar(x, counts, w, bottom=bottoms, color=k_color,
               label=f'K={k_val:.2f}', alpha=0.9)
    ...
    ax.set_ylim(0, 105)
    ax.legend(fontsize=FONT_SIZES['legend'], ncol=4, loc='upper right',
              framealpha=0.9, title='Optimal K')
    ax.grid(axis='y', linestyle='--', alpha=0.3)
    ax.set_axisbelow(True)
    ax.set_title('(c) Optimal K distribution by season',
                 fontsize=FONT_SIZES['subtitle'], fontweight='bold')

    for s_idx, s in enumerate(SEASON_ORDER):
        sv = valid[valid['season'] == s]['best_antecedent_days']
        mean_d = sv.mean()
        ax.text(s_idx, 102, f'days={mean_d:.1f}', ha='center', va='bottom',
                fontsize=8, fontweight='bold')


def main():
    print("=" * 70)
    print("CHART 21: SEASONAL THRESHOLD SUMMARY")
    print("=" * 70)
    ...
    panel_c_k_distribution(axes[2], valid)

    fig.suptitle(f'Seasonal API Rainfall Thresholds — {STUDY_LOCATION}, {STUDY_PERIOD}',
                 fontsize=FONT_SIZES['title'], fontweight='bold', y=1.01)

    plt.tight_layout()

    out_dir = f'{OUTPUT_DIR}/chart_21_seasonal_thresholds'
    os.makedirs(out_dir, exist_ok=True)
    out_path = f'{out_dir}/seasonal_summary.png'
