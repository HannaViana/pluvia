"""
Chart 17: Peak Rainfall Intensity vs API Alert Zones — Per Sub-bacia
One figure per sub-bacia, 2x5 grid of time-step panels showing the exponential
intermediate threshold I = a * exp(b * API) + c and four alert zones.
Mirrors chart 15 but uses per-sub-bacia fitted parameters.
"""
...
def create_alert_zone_chart_for_subbacia(shi_cd, shi_nm, subbacia_events, subbacia_params):
    """Create 2x5 grid of I vs API panels for a single sub-bacia."""
    setup_plot_style()
    ...
    for idx, ts_col in enumerate(TIME_STEP_COLS):
        ...
        valid_mask = subbacia_events[ts_col].notna() & subbacia_events[api_col].notna()
        ts_data = subbacia_events[valid_mask].copy()
    ...
    fig.suptitle(f'Peak Rainfall Intensity vs API — {title_name}\n'
...
def main():
    print("=" * 70)
    print("CHART 17: API ALERT ZONES PER SUB-BACIA")
    print("=" * 70)
    ...
    chart_output_dir = f'{OUTPUT_DIR}/chart_17_api_alert_zones_per_subbacia'
    ...
    for shi_cd in subbacia_ids:
        try:
            ...
            shi_nm = subbacia_params['shi_nm'].iloc[0] if 'shi_nm' in subbacia_params.columns and not subbacia_params.empty else ''

            fig = create_alert_zone_chart_for_subbacia(shi_cd, shi_nm, subbacia_events, subbacia_params)
